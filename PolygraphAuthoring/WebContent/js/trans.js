...
var TRANSACTION_ENTITY_PROPERTY_TYPE_INCREMENT = "DELTA";
...
function createEntityTransactionTableNumOfSets(id){
	...
	var DecButton = createButton("deleteSet_"+id, "−", "add", "padding: 2px 10px 1px 10px;", "decrementNumOfSets("+id+")");
...
function PropertyTypeElement(id, setId, pId) {
    switch (SelectedEntitis[id].sets[setId].type) {
        ...
        case TRANSACTION_ENTITY_TYPE_UPDATE:
            var names = [TRANSACTION_ENTITY_PROPERTY_TYPE_NVU, TRANSACTION_ENTITY_PROPERTY_TYPE_INCREMENT];
            var values = [PROPERTY_TYPE_NVU, PROPERTY_TYPE_INCREMENT];
        	if(SelectedEntitis[id].sets[setId].properties[pId].type === "")
        		SelectedEntitis[id].sets[setId].properties[pId].type = PROPERTY_TYPE_NVU;
        ...
        case TRANSACTION_ENTITY_TYPE_RW:
            var names = [TRANSACTION_ENTITY_PROPERTY_TYPE_READ, TRANSACTION_ENTITY_PROPERTY_TYPE_NVU, TRANSACTION_ENTITY_PROPERTY_TYPE_INCREMENT];
            var values = [PROPERTY_TYPE_READ, PROPERTY_TYPE_NVU, PROPERTY_TYPE_INCREMENT];
        	if(SelectedEntitis[id].sets[setId].properties[pId].type === "")
        		SelectedEntitis[id].sets[setId].properties[pId].type = PROPERTY_TYPE_READ;
        	return createSelect(TRANSACTION_ENTITY_PROPERTY_TYPE + id + "_" + setId + "_" + pId, names, values, null, "width: 108px;margin: 0px;", "changePropertyType(this, "+id+","+setId+","+pId+")", SelectedEntitis[id].sets[setId].properties[pId].type);
...
function loadTrans(tid,tName, textEdit) {
	if(textEdit){
	...
	xhttp.send("tname="+tName+"&tid="+tid+"");
}

function saveTrans(tid, tname){
	document.getElementById("buttonResult").innerHTML = "";
	var tNameObj = document.getElementById("transName");
	var j = {
			"Name": tNameObj.value,
	...
	xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhttp.send("tname="+tname+"&tid="+tid+"&name="+tNameObj.value+"&data="+encodeURIComponent(jsonCode)+"");
}
