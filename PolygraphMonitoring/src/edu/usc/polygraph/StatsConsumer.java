...
public class StatsConsumer {
	...
	private HashMap<String, StaleLog > staleLogs = null;
	public static final int RECORDS_TO_RETRIVE = 100;
	private enum ConsumerType {
		Stale, Stats;
	}
	public  HashMap<String, StaleLog > getStaleLogs(){
		return staleLogs;
	}
	...
	public StatsConsumer(String id, String topic, int partition, String kafkaServer) {
		...
		case 1:
			...
			offset = 0;
			staleLogs= new HashMap<String, StaleLog>();
			break;
		}
		...
		props.put("max.poll.records", RECORDS_TO_RETRIVE);
	...
	public synchronized ConsumerRecords<String, String> poll(long offset1) {
		...
		if (offset1 == 0) {
			...
			consumer.seekToEnd(partitions);
			long off=consumer.position(partitions.get(0));
			if (off-1<0)
				off=1;
			consumer.seek(partitions.get(0), off-1);
		} else {
	...
	public void updateStats() {
		System.out.println("Function updateStats() 91");
	...
	public LogRecord getStaleLog(StaleLog s) {
		System.out.println("Function getStaleLog(StaleLog s) 130");
		ConsumerRecords<String, String> records;
	...
	public ArrayList<LogRecord> getLogs(long offset, String id, long endTime) {
		// System.out.println(String.format("Function getLogs(long offset = %d, String id = %s, long endTime = %d) 141", offset, id, endTime));
		...
		System.out.println("Function getLogs(long offset, long endTime)");
		ArrayList<LogRecord> result = new ArrayList<LogRecord>();
		ConsumerRecords<String, String> records;
		// int count = 1;
		...
		System.out.println("MR1 ... result.size = " + result.size());
		return result;
	...
	public StaleLog getOneLog() {
		System.out.println("Function getOneLog() 188");
		ConsumerRecords<String, String> records;
		records = poll(offset);
		boolean b = true;
		for (ConsumerRecord<String, String> record : records) {
	...
	public String statsToJSON() {
		System.out.println("Function statsToJSON() 205");
		ConsumerRecords<String, String> records;
		int noRecCount=0;
		do {
			records = poll(-2L);
		...
		if (clientsStats != null) {
			for (Stats s : clientsStats) {
				if (s.bucket==null)
					s.bucket="";
				json += comma + String.format("{\"cID\":\"%s\",\"reads\":%d,\"writes\":%d,\"stales\":%d,\"avgSS\":%d,\"maxSS\":%d,\"avgMem\":%d,\"maxMem\":%d,\"pDis\":%d,\"fDis\":%d,\"duration\":\"%s\",\"bucket\":\"%s\"}", "Client-" + s.clientID, s.reads, s.write, s.stales, s.avgSS, s.maxSS, s.avgMem, s.maxMem, s.partialDiscard, s.fullDiscard, s.duration, s.bucket);
				overall.reads += s.reads;
				overall.write += s.write;
				overall.stales += s.stales;
				overall.avgSS += s.avgSS;
				overall.maxSS += s.maxSS;
				overall.avgMem += s.avgMem;
				overall.maxMem += s.maxMem;
				overall.partialDiscard += s.partialDiscard;
				overall.fullDiscard += s.fullDiscard;
				comma = ",";
			}
		}
	...
	public synchronized int fillStale() {
		...
		do {
			...
			for (ConsumerRecord<String, String> record : records) {
				...
				staleLogs.put(record.key(),s);
							offset++;
							
				 if (recordsCount> RECORDS_TO_RETRIVE ){
					 break;
				 }
			}
		} while (!records.isEmpty() && recordsCount < RECORDS_TO_RETRIVE );
		return recordsCount;

	}
	public String staleToJSONTest() {
		System.out.println("Function staleToJSON() 234");
		int recordsCount=0;
		ConsumerRecords<String, String> records;
		do {
			records = poll(offset);
...
//		json += "]}";
	...
	public String staleToJSON() {
		System.out.println("Function staleToJSON() 234old");
		ConsumerRecords<String, String> records;
