...
public class LogRecord implements Comparable<LogRecord> {
	...
	public static LogRecord createLogRecord(String line, boolean decode, boolean dummy) {
		...
		try {
			if (ValidationParams.USE_SEQ) {
				...
				result.id = Utilities.concat(tokens[LogRecordToken.ThreadID.Index], ValidationParams.KEY_SEPERATOR,
						tokens[LogRecordToken.SequenceID.Index]);
				result.startTime = Long.parseLong(tokens[LogRecordToken.StartTime.Index + added]);
				result.actionName=line;
				...
				if (!dummy) {
					...
					result.entities = getEntities(tokens[LogRecordToken.Entities.Index + added], decode);
				}
			} else {
				...
				if (!dummy) {
					...
					result.entities = getEntities(tokens[LogRecordToken2.Entities.Index + added], decode);
				}
			}
		...
		if ((ValidationParams.USE_DATABASE || ValidationParams.useTreeMap)
				&& result.getActionName().equalsIgnoreCase("scan")) {
	...
	private static Entity[] getEntities(String entitiesString, boolean decode) {
		// CUSTOMER 1-3-154
		...
		for (int i = 0; i < allEntityStrings.length; i++) {
			...
			String[] allProperties = entityInfo[EntityToken.Property.Index]
					.split(ValidationParams.PROPERY_SEPERATOR_REGEX);
			...
			for (int j = 0; j < allProperties.length; j++) {
				...
				String pv = propertyInfo[PropertyToken.Value.Index];
				if (decode)
					pv = decode(pv);
				properties[j] = new Property(propertyInfo[PropertyToken.Name.Index], pv,
						propertyInfo[PropertyToken.Type.Index].charAt(0));
	...
	private static String decode(String input) {
		input = input.replaceAll(ValidationParams.ESCAPE_START_CHAR + ValidationParams.KEY_SEPERATOR_NUM
				+ ValidationParams.ESCAPE_END_CHAR, String.valueOf(ValidationParams.KEY_SEPERATOR));
		input = input.replaceAll(
				ValidationParams.ESCAPE_START_CHAR + ValidationParams.RECORD_ATTRIBUTE_SEPERATOR_NUM
						+ ValidationParams.ESCAPE_END_CHAR,
				String.valueOf(ValidationParams.RECORD_ATTRIBUTE_SEPERATOR));
		input = input.replaceAll(ValidationParams.ESCAPE_START_CHAR + ValidationParams.ENTITY_SEPERATOR_NUM
				+ ValidationParams.ESCAPE_END_CHAR, String.valueOf(ValidationParams.ENTITY_SEPERATOR));
		input = input.replaceAll(ValidationParams.ESCAPE_START_CHAR + ValidationParams.PROPERY_SEPERATOR_NUM
				+ ValidationParams.ESCAPE_END_CHAR, String.valueOf(ValidationParams.PROPERY_SEPERATOR));
		input = input.replaceAll(
				ValidationParams.ESCAPE_START_CHAR + ValidationParams.PROPERY_ATTRIBUTE_SEPERATOR_NUM
						+ ValidationParams.ESCAPE_END_CHAR,
				String.valueOf(ValidationParams.PROPERY_ATTRIBUTE_SEPERATOR));
		input = input.replaceAll(
				ValidationParams.ESCAPE_START_CHAR + ValidationParams.ENTITY_ATTRIBUTE_SEPERATOR_NUM
						+ ValidationParams.ESCAPE_END_CHAR,
				String.valueOf(ValidationParams.ENTITY_ATTRIBUTE_SEPERATOR));
		input = input.replaceAll(ValidationParams.ESCAPE_START_CHAR + ValidationParams.ESCAPE_START_NUM
				+ ValidationParams.ESCAPE_END_CHAR, String.valueOf(ValidationParams.ESCAPE_START_CHAR));
		input = input.replaceAll(
				ValidationParams.ESCAPE_START_CHAR + ValidationParams.ESCAPE_END_NUM + ValidationParams.ESCAPE_END_CHAR,
				String.valueOf(ValidationParams.ESCAPE_END_CHAR));

		return input;
	}

	public static String escapeCharacters(String input) {
		input = input.replaceAll(String.valueOf(ValidationParams.ESCAPE_START_CHAR), ValidationParams.ESCAPE_START_CHAR
				+ ValidationParams.ESCAPE_START_NUM + ValidationParams.ESCAPE_END_CHAR);
		input = input.replaceAll("(?<!\\(\\d\\d?)" + String.valueOf(ValidationParams.ESCAPE_END_CHAR),
				ValidationParams.ESCAPE_START_CHAR + ValidationParams.ESCAPE_END_NUM
						+ ValidationParams.ESCAPE_END_CHAR);
		input = input.replaceAll(String.valueOf(ValidationParams.KEY_SEPERATOR), ValidationParams.ESCAPE_START_CHAR
				+ ValidationParams.KEY_SEPERATOR_NUM + ValidationParams.ESCAPE_END_CHAR);
		input = input.replaceAll(String.valueOf(ValidationParams.RECORD_ATTRIBUTE_SEPERATOR),
				ValidationParams.ESCAPE_START_CHAR + ValidationParams.RECORD_ATTRIBUTE_SEPERATOR_NUM
						+ ValidationParams.ESCAPE_END_CHAR);
		input = input.replaceAll(String.valueOf(ValidationParams.ENTITY_SEPERATOR), ValidationParams.ESCAPE_START_CHAR
				+ ValidationParams.ENTITY_SEPERATOR_NUM + ValidationParams.ESCAPE_END_CHAR);
		input = input.replaceAll(String.valueOf(ValidationParams.PROPERY_SEPERATOR), ValidationParams.ESCAPE_START_CHAR
				+ ValidationParams.PROPERY_SEPERATOR_NUM + ValidationParams.ESCAPE_END_CHAR);
		input = input.replaceAll(String.valueOf(ValidationParams.PROPERY_ATTRIBUTE_SEPERATOR),
				ValidationParams.ESCAPE_START_CHAR + ValidationParams.PROPERY_ATTRIBUTE_SEPERATOR_NUM
						+ ValidationParams.ESCAPE_END_CHAR);
		input = input.replaceAll(String.valueOf(ValidationParams.ENTITY_ATTRIBUTE_SEPERATOR),
				ValidationParams.ESCAPE_START_CHAR + ValidationParams.ENTITY_ATTRIBUTE_SEPERATOR_NUM
						+ ValidationParams.ESCAPE_END_CHAR);
	...
	public boolean overlap(LogRecord i) {
		...
		if (endTime >= i.startTime && endTime <= i.endTime)
			return true;
		if (endTime == i.startTime || startTime == i.endTime)
			return true;
		return false;
	}
	...
	public boolean intersect(LogRecord log) {
		if ((ValidationParams.USE_DATABASE || ValidationParams.useTreeMap)
				&& (log.getActionName().equalsIgnoreCase("scan") || this.getActionName().equalsIgnoreCase("scan"))) {
			LogRecord scan = this;
			LogRecord other = log;
			...
			String primaryKey = other.getEntities()[0].getKey();
			if (ValidationParams.OLTP_SCAN) {
				TreeType upper = Validator.createObject(tokens[0]);
				TreeType lower = Validator.createObject(tokens[1]);
				TreeType myobj = Validator.createObject(primaryKey);

				if (myobj.compareTo(lower) > 0 && myobj.compareTo(upper) < 0)
					return true;
				else
					return false;
			} else {
				TreeType myobj = Validator.createObject(primaryKey);
				TreeType lowert = Validator.createObject(tokens[0]);
				int limit = Integer.parseInt(tokens[1]);
				if (myobj.compareTo(lowert) < 0)
					return false;
				if (ValidationParams.scanHoles || !ValidationParams.integerScan) {
					return Validator.isScanRelated(scan, myobj, limit);

				}

				int lower = Integer.parseInt(tokens[0]);
				int primaryKeyint = Integer.parseInt(primaryKey);
				if (primaryKeyint >= lower && primaryKeyint < lower + limit)
					return true;
				else
					return false;
			}
		...
		for (int i = 0; i < entities.length; i++)
			for (int j = 0; j < log.entities.length; j++) {
				if (entities[i].getName().equals(log.entities[j].getName())
						&& entities[i].getKey().equals(log.entities[j].getKey())) {
					boolean r = areRefSameProperties(entities[i], log.entities[j]);
					if (r)
						return true;
				}
			}
		...
		for (int i = 0; i < entity1.getProperties().length; i++) {
			for (int j = 0; j < entity2.getProperties().length; j++) {
				...
				if (entity1.getProperties()[i].getName().equals(entity2.getProperties()[j].getName())) {
					if ((entity1.getProperties()[i].getType() != ValidationParams.NO_READ_UPDATE)
							&& (entity2.getProperties()[j].getType() != ValidationParams.NO_READ_UPDATE))
						return true;
				}
			}
		}
	...
	public static String generateEntitiesLog(Entity[] entities) {
		...
		try {
			...
			for (Entity e : entities) {
				sb.append(concat(e.name, ValidationParams.ENTITY_ATTRIBUTE_SEPERATOR + "", e.key,
						ValidationParams.ENTITY_ATTRIBUTE_SEPERATOR + ""));
				for (Property p : e.getProperties()) {
					sb.append(concat(p.getName(), ValidationParams.PROPERY_ATTRIBUTE_SEPERATOR + "", p.getValue(),
							ValidationParams.PROPERY_ATTRIBUTE_SEPERATOR + "",
							p.getType() + "" + ValidationParams.PROPERY_SEPERATOR + ""));
